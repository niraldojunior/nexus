...
| Resource (`Q-RES`)                 | 10  | —   | —   |
| Service (`Q-SVC`)                  | 6   | —   | —   |
| Arquitetura / Plataforma (`Q-ARQ`) | 9   | 2   | 3   |
...
| Q-ARQ-007     | Estender o RBAC (hoje só em `/v1/users` e histórico Geo) para cada caso de uso de Geo/Resource/Service/Order na camada de serviço, e impor isolamento multi-tenant no adaptador (§4 de security.md). O IdP local emite JWT compatível com o Apigee — a migração troca só o emissor.                                                                                                                    | ✅ RBAC e tenant_id (Resource/Service/Order/Party) entraram nas Fases 2–3 da issue #80; VPD Oracle segue em Q-ARQ-008 | —                         |
| **Q-ARQ-008** | VPD (Virtual Private Database) no Oracle + limpeza de contexto de tenant ao devolver a conexão ao pool (security.md §4.1) — defesa em profundidade além do filtro explícito já implementado no adaptador.                                                                                                                                                                                              | 🟠 Aguardando migração Oracle                                                                                         | Arquitetura + Segurança   |
| **Q-ARQ-009** | TDE (Transparent Data Encryption) nas tablespaces de inventário/ordem e Data Redaction para papéis sem necessidade de ver endereço completo (security.md §6).                                                                                                                                                                                                                                          | 🟠 Aguardando migração Oracle                                                                                         | Arquitetura + Segurança   |
| **Q-ARQ-010** | Particionamento e política de retenção para a volumetria do NFR §1–2 (22M Home Passed, 36M ordens/ano — sem partição, a tabela de ordens passa de 100M de linhas em 3 anos).                                                                                                                                                                                                                           | 🔴 Aberta                                                                                                             | Arquitetura + DBA         |
| **Q-ARQ-011** | Teste de carga contra os alvos de 150 req/s sustentados do NFR §2 — nunca executado; é gate declarado antes de release maior (NFR §9).                                                                                                                                                                                                                                                                 | 🔴 Aberta                                                                                                             | Arquitetura + Performance |
| **Q-ARQ-012** | Circuit breaker para integrações externas (Geonet, provedor de LLM) — NFR §8 pede o padrão; hoje só há timeout, sem breaker.                                                                                                                                                                                                                                                                           | 🔴 Aberta                                                                                                             | Arquitetura               |
| **Q-ARQ-013** | Sessão do IdP local em `localStorage` (JWT de 12h) — XSS rouba a sessão inteira. Migrar para cookie `HttpOnly`+`SameSite` exige adaptar o CORS/proxy do front; vale o custo antes do Apigee entrar?                                                                                                                                                                                                    | 🔴 Aberta                                                                                                             | Arquitetura + Segurança   |
| **Q-ARQ-014** | Caminho MCP/Copilot (`mcpModule.registry.executeTool`) usa um esquema de permissões próprio (`tmf:read`/`tmf:write`, concedido por padrão) e roda sob um `defaultUser` fixo (`createToolContext` em nexus-runtime.ts) — não passa pelo RBAC por papel nem pelo isolamento de tenant que a Fase 2/3 da issue #80 impôs nas rotas HTTP. Qualquer conversa do Copilot pode disparar escrita cross-tenant. | 🔴 Aberta                                                                                                             | Arquitetura + Segurança   |
| **Q-ARQ-015** | Generalizar `Idempotency-Key` para as escritas TMF (NFR §8) — hoje só existe em `GeoProjectRepository.getBulkJobByIdempotencyKey`; POST de Resource/Service/Order/Party não é idempotente.                                                                                                                                                                                                             | 🔴 Aberta                                                                                                             | Arquitetura               |
...
| Q-GEO-012 | Installation Point (PI) como `PhysicalResource` de primeira classe no módulo Resource — `@type=InstallationPoint`, ciclo `projected → built → available → reserved → in_use → decommissioned`, exposto como `ServiceabilityPoint` na TMF645. Hoje não existe nada equivalente no MOD02; `INSTALLATION_POINT` foi aposentado como GeographicSiteSpecification (C11, C6) e o cadastro existente migrou para `CUSTOMER_SITE`, mas o PI como capacidade reservável de rede ainda não tem modelagem. | 🔴 Aberta                                                                                                                                 | Engenharia + Produto      |
| Q-GEO-013 | Filtro de camadas do mapa por `siteRole` roda hoje só no front (`roleByCode` calculado a partir do catálogo de specs em memória, ver `web/src/utils/mapLayers.ts`). Vale a pena mover para o servidor (coluna `site_role` em `geo_map_feature`, hoje só tem `sublabel`) para reduzir payload em zooms muito abertos com muitos sites?                                                                                                                                                           | 🔴 Aberta                                                                                                                                 | Arquitetura + Performance |
