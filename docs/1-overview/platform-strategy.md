...
| RBAC só cobre `/v1/users` e histórico de pesquisa Geo; resto exige apenas autenticação | 🔴 | `Q-ARQ-007` em [`open-questions.md`](open-questions.md) |
...
vêm de relatórios de *market research* comerciais (metodologia não auditável) e devem ser lidos como
...
contra um prazo regulatório de 2033 sem lock-in de inventário legado. `Home Passed` ≡ *domicílio com
rede de água/esgoto disponível* — literalmente o mesmo problema de cobertura-por-endereço que
...
TMF645: *este endereço pode injetar X kW neste alimentador?* — o mesmo formato de Service
Qualification que o Nexus já implementa para viabilidade GPON.

### Tier C — oportunístico

Colocation multi-tenant (a camada de *serviço* sobre um DCIM é uma lacuna real — ver §6), redes de
...
- Federação de identidade OIDC/SAML (o IdP local hoje é laboratório — ver `Q-ARQ-007`)
...
| --- | --- |
| Incumbentes OSS (Amdocs, Netcracker, NOSSIS) | Verticalização por configuração (**C1**/**C9**), não customização de schema. A anti-tese está registrada no próprio levantamento operacional: *"O Geosite é um remendo"* ([`inspirations/geosite-legado.md`](../2-functional-specs/inspirations/geosite-legado.md)) |
...
1. **Vender *sobre* as grandes operadoras, não *contra* elas.** Vivo, Claro e TIM não vão trocar a
...
2. **Sinalizar o conflito comercial em vez de escondê-lo.** Vender para Claro/TIM *como V.tal* — que é
...
**Sequência recomendada:** provar em escala nacional dentro da V.tal (a referência de produção *é* a
...
Registradas em [`open-questions.md`](open-questions.md) como referência única — ver `Q-ARQ-007` para
o item que bloqueia diretamente o Tier 0 deste documento (RBAC e isolamento multi-tenant).
...
| [`open-questions.md`](open-questions.md) | `Q-ARQ-007` e demais questões que bloqueiam este roadmap |
