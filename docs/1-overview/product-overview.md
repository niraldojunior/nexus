...
| 2   | **Resource**              | O que existe                            | TMF634, TMF639, TMF664 | ⚠️ Base implementada; aderência ao HLD 1.13 (inclui catálogo em árvore dinâmica `ResourceCatalog`/`ResourceCatalogNode` com identidade agregada 1:1 folha↔ResourceType, catálogo governado de `ResourceRelationshipType` e regras permitidas `ResourceTypeRelationshipRule`, `ResourceSpecification` via FK `resourceTypeId`, detalhe agregado, Audit Trail, catálogo de estados granulares, portas de splitter no piloto Niterói/Icaraí e conexões físicas Porta→CaboDrop com faróis SID) |
