...
O tipo funcional, fabricante, modelo e topologia exibidos para o recurso são propriedades de catálogo: vêm da `ResourceSpecification` e de suas `resourceSpecificationCharacteristic`, não do discriminador legado copiado na instância. As colunas de fabricante/modelo do exemplar permanecem somente como fallback durante a convergência das cargas.
