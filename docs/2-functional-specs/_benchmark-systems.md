...
Legado primário a substituir. Cobre todos os domínios (Location, OSP, ISP, Network & Services, Provisioning, Reports, Catalogue). O módulo _Network & Services_ já separa "serviços de cliente" / "serviços de rede" (proto CFS/RFS); _Resource Provisioning_ tem Viabilidade GPON.

### Kuwaiba (open-source)

Metamodelo de classes hierárquicas. _Service Manager_ + _Contract Manager_; `GenericService` associado a circuitos via relação `uses`. Não tem split CFS/RFS limpo do SID. Connectivity Manager com path computation.
