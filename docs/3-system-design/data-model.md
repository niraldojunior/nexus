...
|---|---|---|---|
| `geographic_address` | **LIST + HASH subpartition** | `state_or_province` (UF) → hash do **código IBGE do município** | Consulta de viabilidade é regional; *partition pruning* por UF elimina 96% dos dados. A subpartição por município equilibra SP (~30% da base) **e preserva a localidade espacial** — ver §4.5 |
...
| Inserção no índice | Espalhada — *block splits* constantes | Sequencial — inserção na folha mais recente |
...
> consulta a geometria na origem. Ver [`integrations.md`](integrations.md) §7.1 — *o território é do
> Geosite; a planta é do Nexus*.
...
|---|---|---|
| Partição | `LIST` por **UF** | Regiões contíguas; *pruning* natural de 96% dos dados |
...
coluna** dos índices compostos — garante *pruning* por tenant.
