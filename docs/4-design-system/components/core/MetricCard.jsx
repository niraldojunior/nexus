    ...
    >
      ...
      {delta && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 10 }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontWeight: 600, fontSize: 'var(--fs-sm)', color: deltaColor }}>
            {deltaDir === 'down' ? '▾' : '▴'} {delta}
          </span>
          <span style={{ fontSize: 'var(--fs-sm)', color: accent ? 'rgba(255,255,255,0.5)' : 'var(--text-tertiary)' }}>vs. mês anterior</span>
        </div>
      )}
    </div>
  );
