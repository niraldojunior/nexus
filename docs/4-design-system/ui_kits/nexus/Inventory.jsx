...
function Inventory({ onNavigate }) {
  ...
  return (
    <div
      style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 'var(--content-max)' }}
    >
      {/* toolbar */}
      <div
        style={{
      ...
      <div className="vt-card" style={{ overflow: 'hidden', padding: 0 }}>
          ...
          <tbody>
            {rows.map((e) => (
              <tr key={e.id} onClick={() => onNavigate('topology')} style={{ cursor: 'pointer' }}>
                ...
                <td>
                  <Badge tone="neutral">{e.tipo}</Badge>
                </td>
                <td style={{ maxWidth: 240 }}>
                  <span style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>{e.addr}</span>
                </td>
                <td>
                  <StatusPill status={e.status} pulse={e.status === 'online'} />
                </td>
                <td>
                  {e.ports === '—' ? (
                ...
                <td>
                  <span style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>{e.vendor}</span>
                </td>
                <td>
                  <span style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{e.sync}</span>
                </td>
        ...
        >
          <span style={{ fontSize: 12.5, color: 'var(--text-tertiary)' }}>
            {rows.length} de 38,1M recursos
          </span>
          <div style={{ display: 'flex', gap: 6 }}>
            <Button variant="ghost" size="sm" iconLeft={<Icon name="chevron-left" size={14} />}>
              Anterior
            </Button>
            <Button variant="ghost" size="sm" iconRight={<Icon name="chevron-right" size={14} />}>
              Próximo
            </Button>
          </div>
        </div>
