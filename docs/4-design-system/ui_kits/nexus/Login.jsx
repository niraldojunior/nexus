...
function Login({ onLogin }) {
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: '100%' }}>
      {/* Brand panel */}
      <div style={{ background: 'var(--surface-ink)', position: 'relative', overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: 48, color: '#fff' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 12 }}>
          <img src="../../assets/nexus-mark-white.svg" alt="Nexus" style={{ height: 43, width: 43 }} />
          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 34, letterSpacing: '-0.01em', color: '#fff' }}>Nexus</span>
        </div>
        <div style={{ position: 'relative' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 30, lineHeight: 1.15, letterSpacing: '-0.02em', maxWidth: 420 }}>
          ...
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.55)', marginTop: 16, maxWidth: 400, lineHeight: 1.6 }}>
            Inventário Convergente — Infraestrutura Passiva, Rede de Acesso, Equipamentos, Recursos Lógicos, sob arquitetura modular, API-first e padrão TM Forum.
          </p>
        ...
        <div style={{ position: 'relative', fontSize: 12, color: 'rgba(255,255,255,0.35)' }}>Holding V.tal</div>
      </div>

      {/* Form panel */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 48, background: 'var(--surface-card)' }}>
        <div style={{ width: '100%', maxWidth: 360 }}>
          <div style={{ font: 'var(--text-h1)', letterSpacing: 'var(--tracking-snug)', marginBottom: 4 }}></div>
          <p style={{ fontSize: 'var(--fs-body-lg)', color: 'var(--text-tertiary)', marginBottom: 28 }}></p>
          <form onSubmit={(e) => { e.preventDefault(); onLogin(); }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Input label="E-mail" iconLeft={<Icon name="mail" size={16} />} value={email} onChange={(e) => setEmail(e.target.value)} />
