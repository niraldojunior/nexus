...
- **Assistente** (`Assistant.jsx`) — Nova Conversa (hero composer + suggestions), Conversas (history list), transcript thread. Bare screens — no fixed PageHead.
- **Locais** (`Locais.jsx`) — map module: floating search pill, layers panel, entity detail panel with IconTabs, legend ramp. Runs in rail (72px) nav mode; synthetic basemap placeholder stands in for Google Maps.
