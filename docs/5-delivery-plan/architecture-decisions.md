...
| D-LOCAL-002 | Persistência deve ser isolada por portas/adapters. | Firmada | Domínio, casos de uso, contratos TMF e validações não dependem do dialeto do banco atual. |
...
| D-RES-001 | Migração de Resource segue UUID v7 + `_origin`. | MOD02 §34.2 | Decidida | Cobre PhysicalResource, LogicalResource e ResourceSpecification. |
| D-RES-002 | Postes de terceiros são cadastrados no Nexus como fonte de verdade V.tal. | MOD02 §34.1 | Decidida | Sem sincronização com concessionárias no MVP. |
| D-RES-003 | Service Assurance fica externa no MVP, mas consumindo eventos. | MOD02 §34.1 | Decidida | Eventos Resource devem atender SA atual e futura. |
| D-RES-004 | Swap de equipamento usa workflow BPMN no MOD05. | MOD02 §34.1 | Decidida | `/resource/{id}/swap` inicia workflow, não operação manual simples. |
| D-RES-005 | ResourceRelationship é extensível via API. | MOD02 §34.1 | Decidida | RelationshipType é entidade governada. |
...
|---|---|---|---|---|---|
| ADR-PEND-001 | Estratégia de convergência Neon Postgres → Oracle/Kafka/Schema Registry. | Plataforma | Produção corporativa | Manter Neon como laboratório ou iniciar adapters corporativos por onda. | Arquitetura |
