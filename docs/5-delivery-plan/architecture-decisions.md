...
| D-GEO-003 | Catálogo bootstrapado de SiteSpecifications e categorias estruturais.     | Antiga Q-GEO-001             | Parcialmente superada | A categoria `FunctionalGroup` foi descontinuada: o domínio aceita apenas `Region`, `Site` e `SubSite`; a spec legada `FUNCTIONAL_GROUP` é soft-retired (C6). O catálogo continua extensível por API conforme C9. |
...
| D-RES-006 | Refatoração Resource Catalog: Árvore dinâmica e desacoplamento de ResourceType | Issue [#188](https://github.com/niraldojunior/nexus/issues/188) | Decidida | `ResourceCatalog` + `ResourceCatalogNode` (árvore de nós `GROUP` ou `RESOURCE_TYPE`) substituem Category/Layer fixas; `ResourceSpecification` aponta exclusivamente via FK `resource_type_id` (`tmf_resource_type`); tipos canônicos independentes de hierarquia e escopados por tenant (`vtal`). Especialização agregada 1:1 folha↔ResourceType e governança de relações formalizadas na issue [#230](https://github.com/niraldojunior/nexus/issues/230). |
| D-ARQ-005 | Nexus Studio é o control plane de metadados publicado | Issue [#191](https://github.com/niraldojunior/nexus/issues/191) | Decidida | Studio fica separado de Configuração de TI; rotas usam inglês e UI pt-BR. Papéis `studio.reader`, `studio.editor` e `studio.admin` governam consulta, edição de draft e publicação, respectivamente; `platform.admin` mantém acesso implícito. Consumidores operacionais usam somente metadata publicada. |
