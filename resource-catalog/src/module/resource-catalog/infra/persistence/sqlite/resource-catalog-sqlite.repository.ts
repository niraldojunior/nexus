...
export class ResourceCatalogSqliteRepository implements ResourceCatalogRepository {
    ...
    ): Promise<ResourceCatalogSqliteRepository> {
        const instance = new ResourceCatalogSqliteRepository(logger);
    ...
    async create(data: ResourceCatalogModel): Promise<ResourceCatalogModel> {
        ...
        await this.db.set(data.id, item);
        return item;
    }

    async findById(id: string): Promise<ResourceCatalogModel | null> {
        this.logger.info(this.logMetadata, safeStringify({ id }));
        const found = await this.db.get(id);
        return found
            ? this.cloneItem(found as unknown as ResourceCatalogModel)
            : null;
    }
    ...
    async findAll(
        ...
        const items: ResourceCatalogModel[] = [];
        ...
        for await (const [_, value] of iterator(env<string>('CACHE_PREFIX'))) {
            if (value['@type'] !== ResourceType.RESOURCE_CATALOG) {
                continue;
            }
    ...
    async patchById(
        ...
        const cloned = this.cloneItem(next as ResourceCatalogModel);
        await this.db.set(id, cloned);
        this.logger.info(
            { ...this.logMetadata, description: 'RESPONSE - RECEBIDO DO DB' },
