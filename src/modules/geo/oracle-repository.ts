...
export class OracleGeoRepository implements IGeoRepository {
  ...
  public async upsertAddress(address: GeographicAddress): Promise<GeographicAddress> {
    await this.upsertAddressWithoutSearchColumns(address, new Date().toISOString());
  ...
  public async listAddresses(query?: GeographicAddressQuery): Promise<GeographicAddress[]> {
    ...
    if (street) {
      const streetSearch = `LOWER(TRANSLATE(street_name,
    ...
    if (streetNr) {
      conditions.push("LOWER(REPLACE(REPLACE(COALESCE(street_nr, ''), ' ', ''), '-', '')) = ?");
    ...
    if (postcode) {
      conditions.push("REPLACE(REPLACE(COALESCE(postcode, ''), ' ', ''), '-', '') = ?");
  ...
  private async loadSiteRelationshipsBySiteIds(
    ...
    const siteSeed = dialectFor().inlineRows(siteIds, 'v', 'id');
