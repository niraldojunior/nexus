// Projetos de trabalho da página Locais (REQ-MOD01-015). Toca o banco de verdade (Oracle) porque
// GeoProjectRepository e a exclusão em GeoTreeService falam com o DatabaseClient direto — não há
// como exercitar a regra sem SQL real.
...
const createFixture = async (): Promise<{ runtime: NexusRuntime }> => {
...
test.skipIf(!oracleConfigured)(
  'siteCount conta todos os locais vinculados, mesmo Retired',
  async () => {
    const fixture = await createFixture();
    try {
...
test.skipIf(!oracleConfigured)(
  'remove() apaga projeto e vínculos e devolve os ids dos sites, sem tocar o Site em si',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'soft-terminate antes de desvincular (mesma ordem da rota DELETE) deixa o Site Retired',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'transitionProjectSites encerra em massa vários locais de uma vez',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'transitionProjectSites deixa de fora o local com relacionamento ativo (bloqueado)',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'transitionProjectSites não duplica histórico ao rodar de novo sobre locais já encerrados',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'DELETE /v1/geo/projects/:id via rota HTTP: local bloqueado mantém o projeto íntegro',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
...
test.skipIf(!oracleConfigured)(
  'site vinculado a um projeto some de roots() e de search(), mas continua acessível via sitesByIds()',
  async () => {
    const fixture = await createFixture();
    try {
      const spec = await createSiteSpec(fixture.runtime);
