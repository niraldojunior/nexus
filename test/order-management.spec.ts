...
test.skipIf(!oracleConfigured)(
  'TMF645, TMF641 and TMF652 order endpoints qualify and execute orders',
  async () => {
    ...
    try {
      ...
      assert.equal(
        (
          resourceOrder.body as {
            resourceOrderItem: Array<{ resourceResult?: { '@type': string } }>;
          }
        ).resourceOrderItem[0]?.resourceResult?.['@type'],
        'PhysicalResource',
      );
      ...
      assert.equal(
        (qualification.body as { serviceQualificationItem: Array<{ eligibility: string }> })
          .serviceQualificationItem[0]?.eligibility,
        'qualified',
      );
      ...
      const orderItem = (
        order.body as {
          serviceOrderItem: Array<{ serviceResult?: { id: string; '@type': string } }>;
        }
      ).serviceOrderItem[0];
      ...
      assert.ok(
        (events.body as Array<{ eventType: string }>).some(
          (event) => event.eventType === 'ServiceQualificationCreateEvent',
        ),
      );
    } finally {
      await server.stop();
