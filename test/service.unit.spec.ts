...
// Exercises ServiceService directly against the repository (no HTTP layer). Distinct coverage
// from service-repository.oracle.spec.ts, which targets `listActiveSupportingResourceIds`
// specifically (issue #180) — this file covers the general CFS/RFS catalog+inventory lifecycle.
// Skips unless ORACLE_* is configured (see test/test-utils.ts). Run with `npm run test:oracle`.
...
test.skipIf(!oracleConfigured)(
  'ServiceService creates and queries service catalog and inventory records',
  async () => {
    const { client, service, appendEvent, party, site, resource, serviceMap } =
      await setupService();
...
test.skipIf(!oracleConfigured)(
  'ServiceService rejects invalid service permutations and missing references',
  async () => {
    const { client, service } = await setupService();
