...
import { OraclePartyRepository } from '../src/modules/party/oracle-repository.js';
...
test.skipIf(!oracleConfigured)(
  ...
  async () => {
    const client = await getOracleTestClient();
    const service = new PartyService(new OraclePartyRepository(client), stubEventService);
...
test.skipIf(!oracleConfigured)(
  ...
  async () => {
    const client = await getOracleTestClient();
    const service = new ResourceService(new OracleResourceRepository(client), stubEventService);
