...
test.skipIf(!oracleConfigured)(
  'TMF geographic endpoints support read and update by id',
  async () => {
...
test.skipIf(!oracleConfigured)(
  'TMF party and resource endpoints support read, update, delete and relationships',
  async () => {
    const server = createApp({ config: createTestConfig(0), logger: createTestLogger() });
    const port = await server.start();
    try {
      ...
      const partyRole = await requestJson(
        port,
        'POST',
...
test.skipIf(!oracleConfigured)(
  'TMF service endpoints support read, update, delete and relationships',
  async () => {
    const server = createApp({ config: createTestConfig(0), logger: createTestLogger() });
    const port = await server.start();
    try {
      ...
      assert.equal(
        (categoryDelete.body as { id: string }).id,
        (category.body as { id: string }).id,
      );
