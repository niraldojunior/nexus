...
test('vercel config routes API traffic to the concrete server function before SPA fallback', () => {
  ...
  const apiTmfRouteIndex = routes.findIndex(
    (route) =>
      route.src === '/api/tmf-api/(.*)' && route.dest === '/api/server?__nexusPath=/tmf-api/$1',
  );
  const v1RouteIndex = routes.findIndex(
    (route) => route.src === '/v1/(.*)' && route.dest === '/api/server?__nexusPath=/v1/$1',
  );
  ...
  assert.ok(
    apiTmfRouteIndex < fallbackIndex,
    'expected /api/tmf-api route to run before SPA fallback',
  );
