...
/**
 * CRUD de características (`resourceSpecificationCharacteristic`/`resourceTypeCharacteristic`) —
 * espelha `ServiceSpecificationCharacteristicsEditor`, mas sem a divisão negócio/técnico (o `group`
 * de uma característica de recurso é texto livre, não um eixo fixo) e com o `value` de tipo
 * variável (`ResourceCharacteristicRow.valueText` + `valueType`, convertidos só ao salvar — ver
 * `buildCharacteristicPayload`).
 *
 * `lockStructure` (issue #216) trava Nome/Tipo/Descrição/Lista como texto e esconde Adicionar/Remover —
 * usado no modal de especificação, onde o conjunto de características é ditado pelo ResourceType e
 * só o Valor é editável por spec.
 */
export default function ResourceCharacteristicsEditor({
  rows,
  onChange,
  disabled,
  lockStructure,
  ...
  onChange: (next: ResourceCharacteristicRow[]) => void;
  disabled?: boolean;
  ...
  const isStructureLocked = Boolean(lockStructure || disabled);
          ...
          <table className="w-full min-w-[900px] text-left text-[0.82rem]">
            <thead>
              <tr className="border-b border-app-border bg-slate-50 text-app-muted">
                ...
                {!isStructureLocked && <th className="w-10" />}
              </tr>
            </thead>
            <tbody>
              {rows.map((row) => {
                    ...
                    <td className="px-3 py-2">
                      {isStructureLocked ? (
                        ...
                        <select
                          value={row.valueType}
                          onChange={(event) => {
                            const nextType = event.target.value as ResourceCharacteristicRow['valueType'];
                            updateRow(row.key, {
                              valueType: nextType,
