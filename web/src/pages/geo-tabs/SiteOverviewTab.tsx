...
export type SiteOverviewTabProps = {
  ...
  // estático: sem alvo de clique, sem hover, sem cursor de edição. Distinto de
  // `lockedByProjectName`, que é um bloqueio de domínio (RN-007) e continua usando
  // `readOnlyNote` mesmo quando `canEdit` é verdadeiro.
  canEdit: boolean;
  onPatchSite: (patch: {
  ...
  return (
    <div className="grid gap-1">
      ...
      {canEdit ? (
        ...
        <IconInfoRow icon={Building2} hint="Local Pai" value={parentSite?.name ?? 'Nenhum'} />
      )}
      ...
      {canEdit ? (
        ...
        <IconInfoRow
          icon={MapPin}
          hint="Endereço"
          value={address ? formatAddressWithSource(address) : 'Sem endereço'}
        />
      )}
